...
class CloutWork_Admin_Model_Comments extends Zend_Db_Table_Abstract
{
    ...
    public function getComment($id)
    {
        ...
        $query = $adapter->select()
                ...
                ->where($adapter->quoteInto('c.id = ?', $id))
                ->join(array('a' => 'articles'), 'a.id = c.article_id', array('a.title AS article_title'))
                ->limit(1);
    ...
    public function updateComment($comment)
    {
        ...
        return $this->update($data, $this->getAdapter()->quoteInto('id = ?', $comment['id']));
    }
